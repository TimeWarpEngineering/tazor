...
**Target State**:
- Move from `.razor` → `.tazor` file extension
...
**Naming Convention**: `{ComponentPath}_razor.g.cs`
- Example: `Components_Pages_Counter_razor.g.cs` from `Components/Pages/Counter.razor`
- Example: `Components_App_razor.g.cs` from `Components/App.razor`
- Example: `Components__Imports_razor.g.cs` from `Components/_Imports.razor`
...
- User-defined C# code blocks from the .razor file
...
#line hidden
```
- Maps generated code locations to original .razor positions
...
- **Determines**: Component files (`.razor`) vs. View files (`.cshtml`)
...
**Source Mapping**: Maps generated code locations to original .razor positions
- `SourceMapping` records: Original span → Generated span
- Used by debugger to show correct line numbers
- Enables breakpoints in .razor files
...
**Trigger**: Changes to `.razor` or `.cshtml` files

**Pipeline**:
1. Parse analyzer config for Razor settings
2. Collect all `.razor` and `.cshtml` files
3. Filter by import files (`_Imports.razor`, `_ViewImports.cshtml`)
...
└── Microsoft.CodeAnalysis.Razor.Compiler/
    └── Microsoft.NET.Sdk.Razor.SourceGenerators.RazorSourceGenerator/
        ├── Components_App_razor.g.cs
        ├── Components_Pages_Counter_razor.g.cs
        ├── Components__Imports_razor.g.cs
        └── ... (one file per .razor/.cshtml)
```
...
From `Counter.razor`:
```csharp
#pragma checksum "...Counter.razor" "{8829d00f-11b8-4213-878b-770e8597ac16}" "..."
// <auto-generated/>
...
### Changes Needed for `.razor` → `.tazor`

1. **File Detection**
   - Update source generator to look for `*.tazor` files
   - Update MSBuild targets to include `*.tazor` in compilation
   - Modify `_Imports.razor` → `_Imports.tazor` discovery

2. **Naming Convention**
   - Change `{Path}_razor.g.cs` → `{Path}_tazor.g.cs`
